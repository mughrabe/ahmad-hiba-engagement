...
(function () {
  ...
  function initParticles() {
    ...
    const palette = [
      { core: '255, 220, 240', glow: '236, 120, 190' }, // glowing pink
      { core: '210, 230, 255', glow: '110, 160, 255' }, // glowing blue
      { core: '230, 215, 255', glow: '160, 120, 230' }, // glowing violet
      { core: '255, 240, 200', glow: '240, 180, 100' }  // golden firefly
    ];
  ...
  function initPetals() {
    ...
    // Warm petal palette
    const palette = [
      'radial-gradient(ellipse at 50% 0%, rgba(246, 198, 221, 0.9), rgba(217, 106, 168, 0.4) 60%, transparent)',
      'radial-gradient(ellipse at 50% 0%, rgba(214, 196, 248, 0.85), rgba(150, 110, 220, 0.35) 60%, transparent)',
      'radial-gradient(ellipse at 50% 0%, rgba(190, 215, 255, 0.8), rgba(110, 160, 255, 0.3) 60%, transparent)',
      'radial-gradient(ellipse at 50% 0%, rgba(248, 214, 158, 0.85), rgba(224, 168, 94, 0.35) 60%, transparent)'
    ];
    for (let i = 0; i < count; i++) {
  ...
  /* ============================================================
     COUPLE CAROUSEL
     ============================================================ */
  const coupleTrack = document.getElementById('coupleTrack');
  const coupleDots  = document.getElementById('coupleDots');
  const couplePrev  = document.getElementById('coupleNavPrev');
  const coupleNext  = document.getElementById('coupleNavNext');

  if (coupleTrack && coupleDots) {
    const dots   = Array.from(coupleDots.querySelectorAll('.couple-dot'));
    const frames = Array.from(coupleTrack.querySelectorAll('.couple-frame'));

    function scrollToFrame(i) {
      if (!frames[i]) return;
      const frameRect = frames[i].getBoundingClientRect();
      const trackRect = coupleTrack.getBoundingClientRect();
      const offset = frames[i].offsetLeft - (coupleTrack.clientWidth - frames[i].clientWidth) / 2;
      coupleTrack.scrollTo({ left: offset, behavior: 'smooth' });
    }

    function activeIndex() {
      const center = coupleTrack.scrollLeft + coupleTrack.clientWidth / 2;
      let best = 0, bestDist = Infinity;
      frames.forEach((f, i) => {
        const fCenter = f.offsetLeft + f.clientWidth / 2;
        const dist = Math.abs(fCenter - center);
    ...
    if (couplePrev) couplePrev.addEventListener('click', () => {
      scrollToFrame(Math.max(0, activeIndex() - 1));
    });
    if (coupleNext) coupleNext.addEventListener('click', () => {
      scrollToFrame(Math.min(frames.length - 1, activeIndex() + 1));
    });

    let scrollTimer;
    coupleTrack.addEventListener('scroll', () => {
      clearTimeout(scrollTimer);
      scrollTimer = setTimeout(updateDots, 80);
    }, { passive: true });

    // Initialize: center on first frame after layout settles
    setTimeout(() => {
      scrollToFrame(0);
      updateDots();
    }, 100);
  ...
  /* ============================================================
     3D TILT — story panels float and tilt toward the cursor
     ============================================================ */
  const tiltEls = document.querySelectorAll('[data-tilt]');
  const fineMouse = window.matchMedia('(pointer: fine)').matches;
  if (fineMouse && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    tiltEls.forEach((el) => {
      el.addEventListener('mousemove', (e) => {
        const r = el.getBoundingClientRect();
        const x = (e.clientX - r.left) / r.width - 0.5;
        const y = (e.clientY - r.top) / r.height - 0.5;
        el.style.transform = `rotateY(${x * 7}deg) rotateX(${y * -7}deg) translateZ(8px)`;
      });
